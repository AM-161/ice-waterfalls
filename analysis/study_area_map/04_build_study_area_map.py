...
def add_scale_bar(
    ...
    ax.add_patch(
        mpatches.Rectangle(
            (x0 - 0.018 * width, y0 - 0.048 * height),
            (x1 - x0) + 0.036 * width,
            0.096 * height,
            facecolor="white",
            edgecolor="#6f786f",
    ...
    ax.text(x0, y0 + 0.028 * height, "", ha="left", va="bottom", fontsize=7.5, color="#1d2520", zorder=10)
...
def draw_locator(ax, countries: Path, admin1: Path, main_extent: tuple[float, float, float, float]) -> None:
    ...
    ax.add_patch(rect)
    ax.set_title("", loc="left", fontsize=8, fontweight="bold", pad=2)
...
def build_map() -> MapOutputs:
    ...
    sc = draw_points_3857(ax_main, assign, norm, size=18, title="Study Area")
    ...
    ax_locator = ax_main.inset_axes([0.745, 0.545, 0.235, 0.235])
    draw_locator_for_lonlat_bbox(ax_locator, countries, admin1, extent_3857_to_lonlat_bbox(main_extent))
    ...
    fig.legend(
        legend_handles,
        ["Station-assigned icefall", "North Tyrol workflow bbox", "Tyrol boundary"],
        loc="lower left",
