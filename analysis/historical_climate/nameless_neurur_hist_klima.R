...
hist_dir <- file.path(root_dir, "analysis", "historical_climate")
dir_cache <- file.path(hist_dir, "cache")
dir_output <- file.path(hist_dir, "output")
...
download_station_weather <- function(station_id, season_start, season_end, cache_dir) {
  cache_file <- file.path(
    cache_dir,
    sprintf(
      "station_%s_%s_%s.csv",
      station_id,
...
ice_name <- row_uid$icefall_name %||% row_uid$name %||% "Nameless - Neurur"
...
best_season <- as.character(season_summary$season[[1]])
worst_season <- as.character(season_summary$season[[nrow(season_summary)]])
...
path_plot <- file.path(dir_output, "uid181_hist_klima.png")
path_daily <- file.path(dir_output, "uid181_hist_klima_daily.csv")
path_summary <- file.path(dir_output, "uid181_hist_klima_summary.csv")
path_compare <- file.path(dir_output, "uid181_best_vs_worst.png")
