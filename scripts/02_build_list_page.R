...
html_lines <- c(
  ...
  '            <div class="row">',
  '              <label title="Technisches Klettern (A) – z.B. A1 bis A4">A</label>',
  ...
  '            </div>',
  '            <div class="row">',
  '              <label title="Mixed-Klettern (M) – Kombination aus Eis & Fels">M</label>',
  ...
  '            </div>',
  '            <div class="row">',
  '              <label title="Wassereis (WI) – reine Eisbewertungen">WI</label>',
  ...
  '',
  '          <div class="section">',
  '            <div class="section-title">Sonne</div>',
  '            <div class="row">',
  ...
  '            <th data-key="climb_max_tomorrow">Max. Kletterbarkeit morgen (Uhrzeit)</th>',
  '            <th data-key="_last_upload_ts">Letzter Upload</th>',
  '            <th>Details</th>',
  '            <th>Topo</th>',
  '          </tr>',
  '        </thead>',
...
for (i in seq_len(nrow(out))) {
  ...
  detail_html <- c(
    ...
    "    if (!files.length){ elUpStatus.textContent = 'Bitte Datei ausw\u00e4hlen.'; return; }",
    ...
    "      let f = fRaw;",
    "      try {",
    "        elUpStatus.textContent = `Bild ${i+1}/${files.length}: wird komprimiert...`;",
    "        f = await compressImage(fRaw);",
    "      } catch(e) {",
    "        f = fRaw;",
    "      }",
    
    "      elUpStatus.textContent = `Bild ${i+1}/${files.length}: Upload...`;",
    ...
    "    }",
    "",
    "    elUpStatus.textContent = `Upload ok (${success}/${files.length}).`;",
